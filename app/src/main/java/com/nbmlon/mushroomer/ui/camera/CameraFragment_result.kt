...
class CameraFragment_result private constructor(): DialogFragment() {
    companion object {
        ...
        @JvmStatic
        fun getInstance(response : AnalyzeResult) =
    ...
    private var response : AnalyzeResult? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                response = it.getSerializable(ANALYZE_RESULT, AnalyzeResult::class.java)
            }else{
                response = it.getSerializable(ANALYZE_RESULT) as? AnalyzeResult
            }
        }
    }
